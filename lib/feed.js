      ...
      async.parallel(
        handle.feeds.map(function(feed) {
          return function(next1) {
            parser.parseURL(feed.url,function(err,feedData) {
              if (err) {
                next1(err);
              } else if (feed.categories) {
                next1(null,feedData.feed.entries.filter(function(entry) {
                  for(var i = 0; i < feed.categories.length; i++) {
                    if (entry.categories && entry.categories.indexOf(feed.categories[i]) >= 0) {
                      return true;
                    }
                  }
                  return false;
                ...
                next1(null,feedData.feed.entries);
              }
            });
          }
        }),
              ...
              feed.forEach(function(entry) {
                ...
                if (!found) {
                  rssContent.push({
                    ...
                    'hashtags': (entry.categories && entry.categories.length > 0) ? (entry.categories.filter(function(category) { return category.replace }).map(function(category) { return '#' + category.replace(/\s/g, '') })) : null,
                    'shouldTweet': handle.tweetByDefault || false
                  })
                }
              })
            });
        ...
        rssContent.map(function(item) {
          return function(next1) {
            if (!item.tweet) {
              item.tweet = '';
              if (handle.prefix) {
                item.tweet += handle.prefix;
              }
              item.tweet += item.title;
              if (item.hashtags) {
                item.hashtags.forEach(function(hashtag) {
                  if ((item.tweet + ' ' + hashtag + (item.bitlyUrl || item.url)).length < 140) {
