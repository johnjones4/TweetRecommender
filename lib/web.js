...
exports.init = function(_handles) {
  ...
  var auth = basicAuth(config.auth.username, config.auth.password);

  app.get('/', auth, function(req,res) {
    res.render('index',{});
  });

  app.get('/handle', auth, function(req,res,next) {
    async.series(
  ...
  app.get('/handle/:slug', auth, function(req,res,next) {
    if (req.params.slug) {
  ...
  app.put('/handle/:slug', auth, function(req,res,next) {
    if (req.params.slug) {
      var handle = handles.find(function(handle) {
        return handle.slug == req.params.slug;
      });
